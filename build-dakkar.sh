    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            aosp80)
                ...
                treble_generate=""
                extra_make_options=""
                extra_make_options_origin="ANDROID_COMPILE_WITH_JACK:=false"
                ;;
            aosp81)
                mainrepo="https://android.googlesource.com/platform/manifest.git"
...
function build_variant() {
    lunch "$1"
    make $extra_make_options $extra_make_options_origin BUILD_NUMBER="$rom_fp" installclean
    make $extra_make_options $extra_make_options_origin BUILD_NUMBER="$rom_fp" -j "$jobs" systemimage
    make $extra_make_options $extra_make_options_origin BUILD_NUMBER="$rom_fp" vndk-test-sepolicy
